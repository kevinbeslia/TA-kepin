...
$query2 = mysqli_query($konek,"SELECT * FROM tbl_kriteria");
	$rq2 = $query2->num_rows;
	for ($i=1; $i <= $rq2 ; $i++) { 
        $sqlceknilai = $konek->query("SELECT * FROM tbl_penilaian a JOIN tbl_subkriteria b ON a.id_subkriteria = b.id_subkriteria JOIN tbl_kriteria c ON b.id_kriteria = c.id_kriteria WHERE b.id_kriteria = '$i' AND id_alternatif = '$ids'");
